...
DEFAULT_OUTPUT = ROOT / "results" / "q1_1" / "fraction_recalculation_audit" / "v2"
AUDIT_SCHEMA_VERSION = "q1-1-frac-recalculation-audit-v2"
...
def recalculate_record(record: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    ascii_alpha_count = sum(("A" <= ch <= "Z") or ("a" <= ch <= "z") for ch in text)
    out["calc_complement_of_ascii_english_letter_character_fraction_pct"] = (
        100.0 * round((len(text) - ascii_alpha_count) / len(text), UPSTREAM_PRECISION)
        if text
        else None
    )
...
def candidates_for_field(field: str) -> List[Tuple[str, str]]:
    ...
    if field == "rps_doc_frac_no_alph_words":
        return [
            ("public_word_level_formula_x100", "calc_rps_doc_frac_no_alph_words_pct"),
            (
                "complement_of_ascii_english_letter_character_fraction_candidate_x100",
