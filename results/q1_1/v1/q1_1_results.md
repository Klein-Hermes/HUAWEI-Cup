...
4. 本报告记录 Q1.1 原始全量评分计算完成时的状态：当时原计划的 793 条 A1 全量双人盲评尚未完成，因此本计算报告不据此选择唯一操作主 Q。此后完成的 30 条双人微型核查也未区分 `q_equal` 与 `q_huber`；项目负责人随后将 `q_huber` 冻结为操作主 Q、`q_equal` 保留为敏感性分。这是操作性选择，不是人工评审胜出；793 条全量盲评仍未完成。当前决策见 `Q1/03_结果/Q1_最终收口/q1_final_decision.md`，全量评审状态见 `Q1/03_结果/Q1.1/review_intake/v1/closeout_status.md`。
