...
def fig_rank_robustness():
    ...
    scens_all = [s for s in SCEN_LABEL if s in set(df.scenario) and s != "weight_alpha_1"]
    scens_all = sorted(scens_all, key=lambda s: df[df.scenario == s]
                       ["seven_domain_rank_spearman"].iloc[0])

    # 十个场景里只有剔除 7 字段这一个动了排名，其余九个秩位移全为 0、秩相关恒为 1。
    # 原先画成 10×7 热图，那 9 行在 RdBu 色标下渲染成整片纯白，读者分不清
    # "位移为 0" 与 "没有数据"；改成只画有信息的那个场景，其余用文字交代。
    sc = scens_all[0]
    rho = df[df.scenario == sc]["seven_domain_rank_spearman"].iloc[0]
    sub = df[df.scenario == sc].set_index("source_domain")
    vals = [float(sub.loc[d, "rank_shift"]) if d in sub.index else 0.0 for d in doms]
    n_moved = sum(1 for v in vals if v != 0)

    fig, ax = plt.subplots(figsize=(7.0, 3.2))
    S.style_axes(ax, grid_axis="y")
    x = np.arange(len(doms))
    cols = [S.C["hl"] if v > 0 else S.C["ours"] if v < 0 else S.C["ref"]
            for v in vals]
    ax.bar(x, vals, width=0.56, color=cols, edgecolor="white", lw=0.8, zorder=3)
    for xi, v in zip(x, vals):
        if v == 0:
            ax.text(xi, 0.06, "0", ha="center", va="bottom", fontsize=8.4,
                    color=S.C["ref"])
        else:
            off = 0.09 if v > 0 else -0.09
            ax.text(xi, v + off, f"{int(round(v)):+d}", ha="center",
                    va="bottom" if v > 0 else "top", fontsize=8.6,
                    color=S.C["hl"] if v > 0 else S.C["ours"])
    ax.axhline(0, color=S.C["ref"], lw=0.9, zorder=2)
    ax.set_xticks(x, doms, fontsize=8.8)
    lim = max(abs(min(vals)), abs(max(vals))) + 1.0
    ax.set_ylim(-lim, lim)
    ax.set_ylabel("秩位移（正 = 排名下降）")
    ax.text(0.985, 0.95,
            f"场景：{SCEN_LABEL[sc]}（$\\rho={rho:.2f}$）\n"
            f"其余 {len(scens_all)-1} 个场景秩位移全为 $0$、$\\rho=1.00$，未绘出",
