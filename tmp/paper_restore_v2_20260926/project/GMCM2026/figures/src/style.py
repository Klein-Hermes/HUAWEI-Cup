...
def setup():
    plt.rcParams.update({
        ...
        "font.sans-serif": ["Songti SC", "Heiti SC", "PingFang SC",
...
# 文字压数据的自动检测
# ----------------------------------------------------------------------
def check_text_overlap(fig, thresh=0.03, check_text_on_text=True):
    """检测图内文字是否压在数据上，以及两处文字是否互相压叠。

    第一类（文字压数据）的做法：先正常渲染取得所有文字的像素包围盒，再把文字
    全部隐藏后重渲染，在「无文字」图上统计每个包围盒内的非白像素比例——非白即
    代表该处有线、点、柱或填充。隐藏文字不改变版面，故两次渲染的包围盒位置可比。

    第二类（文字压文字）是上面做法的盲区：它把**所有**文字都隐藏了，因此「图例
    文字盖住某个数值标签」这类重叠完全查不出来（2026-09-25 修图时正是这样漏掉
    两张图）。补法是逐个候选文字单独隐藏、其余文字保持可见，再看该文字的包围盒
    里是否出现深色像素——有，就说明下面压着另一处文字。
    """
    fig.canvas.draw()
    ...
    # 第二类：逐个候选文字单独隐藏、其余保持可见，看它下面是否压着别的文字
    if check_text_on_text:
        for t in items:
            t.set_visible(False)
            fig.canvas.draw()
            buf2 = np.asarray(fig.canvas.buffer_rgba())[..., :3].copy()
            t.set_visible(True)
            try:
                bb = t.get_window_extent(renderer=rend)
            except Exception:
            ...
            if x1 <= x0 or y1 <= y0:
                continue
            patch = buf2[y0:y1, x0:x1]
            if patch.size == 0:
                continue
            # 只算「深色」像素：那是另一处文字的笔画，而不是底图图形
            frac = float((patch < 140).all(axis=-1).mean())
            if frac > 0.05:
                hits.append(("【被别的文字压住】" + t.get_text().replace("\n", " ")[:30],
                             frac))
        fig.canvas.draw()
    return hits
...
def save(fig, name, vector=True):
    """导出到 figures/。位图 600dpi + 矢量 PDF 双份。
