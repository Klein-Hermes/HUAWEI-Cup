...
大语言模型训练需在参数量、数据量、数据质量与领域配比之间分配有限算力。本文依据附件 A 的质量信号与配方实验、附件 B 的训练轨迹及附件 C 的能力评测，建立质量评分、配比—损失分析、广义标度律与算力约束模型，并以数据可比性和效应可识别性审查限定结论。

\textbf{问题一：}对 22 个量纲与方向不同的质量指标，在 A1 上拟合经验分位映射并统一方向，以完整度与域排序稳定性构造权重，采用 Huber 位置形成操作评分。A1 七域宏平均为 $0.498478$（$95\%$ 区间 $[0.496567,0.500427]$）；A2/A3 同域差值区间均跨零，指标对排序相关为 $0.99963$ 和 $0.99949$。冲突度定义为 231 个指标对的平均绝对分差，域内经验 $q_{95}$ 作为描述性阈值，实际超阈率按有限样本报告。配比—损失模型采用单纯形岭回归，A4/A5 分组交叉验证误差为 $0.39860$，略优于参考 OLS 的 $0.40032$；A6/A7 同尺度检验得 $R^2=0.6189$、Spearman $=0.8327$。跨至 60M 和 1B 后 $R^2$ 降至 $-7.9336$ 和 $-810.12$，故只支持同尺度配比筛选。配比派生质量特征在 478 个共同 OOF 样本上的标准化 RMSE 由 $0.59517$ 升至 $0.76335$，不据此声称存在独立质量效应。

\textbf{问题二：}以 $D_{\mathrm{eff}}=D\exp\{h(Q,p)\}$ 构造广义标度律，并用参考态约束保留经典形式。B1 经典分支估得 $E=1.689798$、$A=0.353980$、$\alpha=0.339977$、$B=1.240306$、$\beta=0.279878$，内样本 $R^2=0.99999982$；中位点弹性为 $\varepsilon_N=-0.048225$、$\varepsilon_D=-0.036924$。附件 B 中可逐行关联到损失的 17 维配比记录为零，故配比和质量扩展效应未估计，不解释为零效应。

\textbf{问题三：}按训练、质量提升与长上下文注意力三类成本建立约束模型，解析得到两类成本相等的临界上下文长度 $30000$。冻结 B1 网格上的 15 个预算—上下文情景均有可行点；$10^{22}$ 档预算利用率为 $98.92\%$--$99.64\%$，$10^{24}$ 档因 1176 个网格点均可行而受上界限制。注意力成本份额在上下文长度 2048 与 131072 时分别为 $6.390\%$ 和 $81.375\%$。

\textbf{问题四：}本稿尚未完成附件 C 的实证分解与前沿预测，故不报告未经数据支持的贡献占比或预测值。第九章汇总已完成的字段与权重敏感性、整轨迹 bootstrap 稳定性和外推边界；其中标度律重抽样仅基于 8 条 B1 轨迹，作为既定模型与支持范围内的稳定性提示。主要限制是质量评分受 7 个语义未追溯字段影响，配比与质量的独立效应不可识别，配比模型跨尺度定量外推失效。
...
\begin{tabularx}{\textwidth}{lXX}
  ...
  队伍需依据原始程序、登记表与结果文件逐项复核，确认最终稿数字和口径 \\
  论文审阅与排版 &
  按审查清单核对已写稿件、引用、表述与 LaTeX 编译结果 &
  队伍复核每项修改并对模型、数据和最终提交内容负责 \\
  \bottomrule
\end{tabularx}
...
\begin{enumerate}
  ...
  \item \textbf{独立复算}：凡涉及数值结果的内容，
        本次审阅未运行或重算原论文的建模实验；本稿内未附完整程序代码，
        因此不能仅凭本文档独立复现全部数值。定稿前应由参赛队结合原始程序、数字登记表
        与结果文件逐项复算，并确认正文、图表与数据口径一致。
...
本次审阅与改稿中，Codex 用于按清单审查已写内容、核对引用和表述、修改 LaTeX
源文件并检查编译结果；本次会话没有重算论文所述实验，也没有生成新的模型结果。
参赛队应核对模型推导、程序、数据来源与数值登记，对所有修改作最终判断，并对提交稿
的正确性与合规性负责。
