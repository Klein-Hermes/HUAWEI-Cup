...
3. Hugging Face v2 `contents` 历史有 3,598 次提交，当前固定版本为 `9c09a7cae43334062a82cb164f2ef255013dafa2`（`2025-03-20T12:17:27Z`）。另从 v2 仓库历史重建 10 个逐月最后可用版本，其中 1/9 个可计算的当月 P90 与冻结最终表一致；缺少 `Submission Date` 的月份：2024-06。最终值差异按共同模型名与 `Model sha` 进一步拆分，见 `historical_snapshot_report.md`。该源不含 2026-03 观测。v1 的六项任务与 v2 六个任务族不同，不能把 v1 的 Average/P90 接到 v2 时间序列。
4. C1/C2 完整字段对齐：**4,576/4,576** 一对一匹配。远程 v2 全行核验：**4,576/4,576** 行一对一匹配；上游 SHA 为 `9c09a7cae43334062a82cb164f2ef255013dafa2`，最后更新 `2025-03-20T12:17:27.000Z`。
5. 重建得到 **201** 条“明确开放权重”记录，覆盖 10 个有数据月份；每月样本数为 4–38。其中 2 个月 n<10、7 个月 n<20；若采用 n≥20 才纳入动态估计，仅剩 3 个月，不能支持年度预测。重建 P90 与冻结表 `10/10` 月一致。
6. 当前 C2 的 Epoch 开放权重标签没有随 v2 月度快照版本化，因此历史月份结果只能称“用当前标签回溯定义的描述性队列”，不能充当无前视偏差的开放状态回测。在 1,819 个冻结的 C8 精确名称/C1 唯一候选中，开放权重样本有 198 条与候选表登记的 C8 最新模型 SHA 完全一致，2 条不一致，1 条缺少 C1 来源 SHA。SHA 相同只确认权重身份；任务配置与指标定义仍需单独核验。
