...
- 分析固定六族完整样本中的 **40** 个模型及 BBH **24** 个 `acc_norm` 叶任务。完整六族既定指数与六族得分等权平均的最大差为 **1.42e-14**。
...
- 逐个剔除任务后，Spearman 名次相关的最小值为 **0.9994**（剔除 `leaderboard_bbh_disambiguation_qa`），24 项诊断的中位数为 **0.9998**。
- 基线 Top-10 在逐任务剔除后的最小保留率为 **100.0%**（剔除 `leaderboard_bbh_boolean_expressions`）。
- 六族指数点数变化的最大绝对值为 **0.4767**，对应剔除 `leaderboard_bbh_tracking_shuffled_objects_three_objects`。
- 最大单模型名次位移为 **2** 位，发生在剔除 `leaderboard_bbh_disambiguation_qa`；模型 `Ateron/Lotus-Magpic`。
- 在完整六族样本中，BBH 任务原始分数中位数最低/最高的叶任务分别是 `leaderboard_bbh_tracking_shuffled_objects_seven_objects`（0.1500）和 `leaderboard_bbh_boolean_expressions`（0.8020）。此处“难/易”只表示本队列的描述性中位数，不表示题目难度的普遍属性。
