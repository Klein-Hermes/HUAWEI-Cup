...
def main() -> None:
    ...
    license_audit_path = RESULTS / "q4_license_source_audit.csv"
    write_csv(registry_path, registry_rows)
    write_csv(audit_path, audit_rows)
    license_audit_rows = []
    for row in registry_rows:
        if row.get("provisional_unique_name_candidate") != "True":
            continue
        license_text = row.get("c1_hub_license_raw") or ""
        license_text_normalized = license_text.strip()
        open_weights_raw = row.get("c2_epoch_open_weights") or ""
        open_weights_normalized = open_weights_raw.strip()
        license_audit_rows.append({
            "registry_id": row.get("registry_id", ""),
            "model_name_raw": row.get("model_name_raw", ""),
            "version_match_status": row.get("version_match_status", ""),
            "strict_version_confirmed": row.get("strict_version_confirmed", "False"),
            "c2_epoch_open_weights_raw": open_weights_raw,
            "c2_epoch_open_weights_status": open_weights_normalized if open_weights_normalized in {"Yes", "No"} else "unknown",
            "c1_hub_license_raw": license_text,
            "c1_license_field_status": "recorded_unverified" if license_text_normalized else "blank",
            "license_terms_independently_verified": "False",
            "research_reproduction_permission_status": "not_assessed",
        })
    write_csv(license_audit_path, license_audit_rows)
    ...
    license_recorded = sum(bool((row.get("c1_hub_license_raw") or "").strip()) for row in license_audit_rows)
    license_blank = len(license_audit_rows) - license_recorded
    open_yes_count = sum(row.get("c2_epoch_open_weights_status") == "Yes" for row in license_audit_rows)
    open_yes_license_blank = sum(
        row.get("c2_epoch_open_weights_status") == "Yes" and row.get("c1_license_field_status") == "blank"
        for row in license_audit_rows
    )
    ...
    report = [
        "# Q4 覆盖率与有效样本报告", "",
        "> AI 辅助说明：OpenAI Codex 桌面版 26.917.71314（build 10954，prod），模型 ID `gpt-6-luna`（GPT-6 Luna），开发机构 OpenAI，发布日期 2026-09-22。数据字段、合并和结论须由队伍逐项复核。", "",
        ...
        "## License 字段来源覆盖", "",
        f"1,819 个唯一名称候选中，C1 `Hub License` 原始字段有值 {license_recorded} 条、为空 {license_blank} 条；C2 `Open Weights=Yes` 有 {open_yes_count} 条，其中 C1 License 字段为空 {open_yes_license_blank} 条。逐候选原值见 `../03_结果/results/q4_license_source_audit.csv`。", "",
        "该表只审计字段覆盖并保留原始文本；没有逐个核验许可证条款、版本或研究/复现权限。`Open Weights=Yes` 只作为队列定义中的权重开放字段，不能解释为许可证授权结论。", "",
        "## 时间与规模样本", "", month_md, "",
        ...
        "- `../03_结果/results/q4_license_source_audit.csv`：唯一名称候选的 C1 License 原值、C2 Open Weights 原值及许可核验状态；不构成法律判断。",
    ...
    outputs = [registry_path, audit_path, license_audit_path, coverage_path, report_path]
    manifest = {
        "status": "provisional_user_decision_accepted_ai_scope_review_pending",
        "generated_at_local": datetime.now().astimezone().isoformat(timespec="seconds"),
        "ai_disclosure": "OpenAI Codex desktop 26.917.71314 (build 10954; prod); model ID gpt-6-luna (GPT-6 Luna); developer OpenAI; official release date 2026-09-22.",
        ...
        "limitations": [
            ...
            "The license-source audit preserves raw C1 values and missingness; no license terms or research/reproduction permissions were independently verified.",
