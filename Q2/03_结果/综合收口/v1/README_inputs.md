...
清单当前状态：106 条记录；89 条与冻结哈希匹配；15 条有当前哈希但没有历史冻结哈希可供比对；1 条较历史哈希有变化；1 条在历史登记路径缺失。`HASH_MATCH` 仅表示当前内容与该项已登记的冻结哈希一致；`AVAILABLE_CURRENT_HASH_RECORDED` 不代表历史版本匹配。差异明细见 CSV 的 `current_status` 列。
