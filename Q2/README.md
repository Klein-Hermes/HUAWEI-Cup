...
**当前数据版本状态：`FROZEN_WITH_LIMITATIONS`。** M0 与验证/识别审计已收口；B 侧 p 门禁为 `FAIL_CURRENT_DATA`，Q 独立增量门禁为 `FAIL_CURRENT_EVIDENCE`，M1/M2 均为 `NOT_ESTIMATED`。这表示当前证据版本已冻结，不表示广义 `Loss=f(N,D,p,Q)` 已完成估计；出现满足门禁的新证据时可重新开启。
...
- 03_结果/p配比可行性审计/v1/、v2/：v1 为首轮审计快照；当前恢复核查与门禁结论以 v2 为准，报告、轨迹映射、来源证据和哈希清单均在 v2/。
...
- 04_图表/q2_supplementary/：三张补充逻辑图的独立展示版本；含完整注释版与论文版，不覆盖冻结收口包中的原图。生成回执见 `05_审计交付/Q2补充绘图任务单_完成回执.md`，复现入口见 `src/q2_supplementary_figures.py`。
- 05_审计交付/：B4/B5 专审交付副本、逐组 provenance 待核表和原始论文网址清单；权威版本以 03_结果 中的审计文件为准。

## 关键报告

- **当前具体作答稿：**[问题二逐项作答与实证结果（v4）](03_结果/最低限度题目答复/v4/Q2_具体作答稿_20260926.md)。正文已列出 B1 基线参数与数值边际效应、A 侧配比验证及 A12–A15 估算表一致性、B2–B5 分源验证边界、B6/B7/B8 质量响应、等 Loss 条件与 B9/B10 情景；完整 B 侧联合经验估计未完成的部分明确标为未估计。
...
- 当前 p 缺口恢复审计：03_结果/p配比可行性审计/v2/q2_p_gap_resolution_report.md（B 侧实测 p–Loss 联接仍为 0，M1 未估计）
...
从项目根目录运行：
    ...
    & "D:\Anaconda\python.exe" src/f_q2_p_feasibility_audit.py
...
`01_方案说明/Q2_p配比可行性审计.md` 是项目 `docs/Q2_p配比可行性审计.md` 的副本；首轮审计快照见 `03_结果/p配比可行性审计/v1/`，当前恢复核查与结论见 `v2/`。
...
详细门禁证据见本节所列的综合收口产物；该冻结不表示 p 或 Q 无效，有新的可追溯 B 侧联合数据时可重开门禁。
...
- 条件性作答稿（联合经验估计未完成）：`03_结果/最低限度题目答复/v1/Q2_最低限度作答稿_现有附件边界.md`（复用已冻结结果，汇总广义模型、分来源边际效应、弹性和等 Loss 替代关系，并明确未完成共同样本上的联合经验估计）
