...
这不等于完整 Q2 已完成。当前 B 组训练表可运行 (M_0=f(N,D))，但没有可直接联接的领域配比 \(\boldsymbol p\)；Q1.1 正式操作主 Q 已冻结为 `q_huber`（版本 `Q1-q_huber-v1`），`q_equal` 保留作敏感性对照。框架保留 (M_1=f(N,D,\boldsymbol p)) 与 (M_2=f(N,D,\boldsymbol p,Q)) 的同一调用接口，并硬性拒绝候选 Q 进入正式 M2。**M1 不依赖最终 Q；当前不能运行 M1 的原因是 B 组缺少逐样本 p，且尚无 Q2 的 M1 拟合器。**框架不生成或假定 p、Q。

模型计数上，M0/M1/M2 是题目规定的同一广义标度律内逐步放开的嵌套状态（分别不含配比、仅含配比、含配比与质量），不算三个独立模型体系。最多再加入一个预先定义的交互扩展；该扩展只有在新增联合观测能识别交互项时才启动。
...
有逐样本 p 时，可用 `Q2Dataset` 和共同 splits 调用 M1；M1 本身不依赖 Q。以下示例中的 `fit_m1` 需先按 Q2 主方案实现：
...
有逐样本 p 后，即可在不加载 Q 的情况下，对同一共同完整样本集比较 M0 与 M1，并按训练轨迹分组生成、复用相同的 splits。M2 应加载 `q_status="final"`、`q_version="Q1-q_huber-v1"` 的正式来源，再构造 M0/M1/M2 的共同完整样本集并让三种模型复用一份 splits。`q_equal` 只作为敏感性对照单独报告；候选 Q 或缺少版本号的 Q 不得写入正式 M2 结果。
...
这些输入到位后，M1 可在不加载 Q 的情况下运行；正式 Q 已冻结为 `Q1-q_huber-v1`，但涉及 Q 效应、p×Q 或质量—规模替代分析的 M2 仍需独立质量变化证据与可用的 p 数据链。若仍没有可信的逐样本 p，正确结论是“Q2 p-only 数据受限”，而不是用 Q1.3 的聚合结果代替训练配方。

## 当前边界

- P1 仅覆盖 B1/M0 的共享切分、预测、指标、组级 Bootstrap 和文件写出。
- `p_only` 与 `p_plus_Q` 目录及接口已预留，暂不运行；当前既缺可追溯的逐样本 p 联接，也缺 Q2 的 `fit_m1`。M1 不需要 Q；M2 可读取已冻结的 `Q1-q_huber-v1`，但要估计独立 Q 效应仍需独立质量变化证据和可用的 p 数据链。
