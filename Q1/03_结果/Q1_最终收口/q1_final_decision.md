...
**G3 本轮收口：`CLOSED_WITH_LIMITATIONS`。** 保留现有 30 条译文双评结果作为有限一致性证据；原文评分可靠性验证仍未完成，不将 G3 标记为通过。题面没有要求对 793 条做全量人工评分，因此本轮不追加该项工作。论文中须披露译文、样本范围、评审顺序限制及 A2/A3 原文映射限制。
...
| 层面 | 判断 |
|---|---|
| Q1.1 候选分计算、稳健性与人工微型评审分析 | 已完成带限制收口；G3 原文验证未通过，30 条证据仅适用于译文一致性核查 |
