...
> **历史范围说明（2026-09-24 更新）：**本文记录的是原计划 793 条 A1 全量盲评包的来源追踪。末尾“主模型选择未完成”仅指该全量评审方案当时尚未回收，不代表当前 Q1 总体状态。之后 30 条双人微型核查已完成，负责人已冻结 `q_huber` 为操作主 Q（`Q1-q_huber-v1`），`q_equal` 为敏感性分；全量 793 条评审及 A2/A3 原文映射仍未完成。当前正式状态见 `../../../Q1_最终收口/q1_final_decision.md`。
...
本地文件可以恢复一部分 A2/A3 原文映射。对 A2 的 17,523 个 arXiv ID 和 A3 的 203,752 个 GitHub ID，与含原文的 A1 文件按精确 ID 交叉核对后，分别找到 1,419 条和 10,000 条。所有匹配行的来源域均一致；七个 RPS 数值也逐字段一致（绝对误差不超过 1e-9），未发现匹配行数值不一致。对应文本可由 A1 原文文件按 ID 取回。部分映射表位于 `results/q1_1/restricted_source_trace/a2_a3_partial_text_linkage.csv`，只含 ID、来源路径和核对状态，不含文本；请勿发给盲评者。仍有 A2 的 16,104 条、A3 的 193,752 条未能从当前 A1 原文样本映射，因此不能声称 A2/A3 全量原文映射恢复。原计划的 793 条盲评只覆盖 A1；另行完成的 30 条微型核查见 `human_spotcheck_final/received_20260924/`，不补足 A2/A3 映射。
...
原计划的 793 条全量盲评表仍为空，因此不能声称已完成 793 条人工验证。另行完成的 30 条微型核查没有区分候选；项目负责人随后作出透明的操作性选择，冻结 `q_huber` 为正式主 Q、`q_equal` 为敏感性分。这个决定不等同于人工评审选出胜者，也不改变 A2/A3 原文映射限制。
