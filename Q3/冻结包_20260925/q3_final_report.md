...
当前证据支持的是：在固定 $Q=Q_0$、冻结 M0、B1 精确实测 N/D 支持网格上的经验参考优化与成本/可行性审计；在相同范围内描述 Bootstrap 参数敏感性及相邻情景的离散变化；并在未识别 $\beta_Q$ 的条件假设下给出 T4 阈值敏感性。它们分别回答有限范围的问题，不能合并解释为完整 Q3 的联合优化结论。
