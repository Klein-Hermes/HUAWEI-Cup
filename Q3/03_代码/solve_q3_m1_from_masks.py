...
OUT_FINAL_MASK = Path("Q3/04_结果/m1_final_feasible_mask.csv")
...
def solve_m1(
    ...
    for budget, context in scenarios:
            ...
            final_mask_rows.append(
                {
                    ...
                    "m0_predicted_val_loss": candidate["m0_predicted_val_loss"],
                    "m3_search_allowed": m3_allowed,
                    "budget_feasible": budget_feasible,
                    "final_feasible": final_feasible,
                    "support_class": support["support_class"],
                    "inside_convex_hull": support["inside_convex_hull"],
                    "interpolation_candidate": support["interpolation_candidate"],
                    "extrapolation_flag": support["extrapolation_flag"],
                    "nearest_observed_distance_standardized_log": support[
                        "nearest_observed_distance_standardized_log"
                    ],
                    "C_train_flops": mask["C_train_flops"],
                    "C_Q_flops": mask["C_Q_flops"],
                    "C_attn_flops": mask["C_attn_flops"],
                    "C_total_flops": mask["C_total_flops"],
                    "budget_slack_flops": mask["budget_slack_flops"],
                }
            )
            if budget_feasible:
        ...
        selected_final_feasible = (
            support["m3_search_allowed"].lower() == "true"
            and mask["budget_feasible"].lower() == "true"
        )
        results.append(
            {
                "scenario_id": mask["scenario_id"],
                ...
                "final_feasible": selected_final_feasible,
    ...
    return results, final_mask_rows
...
def main() -> int:
    ...
    results, final_mask_rows = solve_m1(candidates, support_by_id, masks)
    ...
    write_csv(ROOT / OUT_FINAL_MASK, final_mask_rows)
    ...
    outputs = [OUT_FINAL_MASK, OUT_BOUNDARY, OUT_REPORT]
    manifest = {
