...
当前可识别的主结果是：固定 $Q=Q_0$，在 B1 的 1,176 个实测 $(N,D)$ 候选点中，以冻结 M0 预测 Loss 选择预算参考配置。上下文为 2,048 tokens 时，三个预算对应：

| 预算 (FLOPs) | $N^*$ (B) | $D^*$ (B) | M0 预测 Loss | 预算利用率 |
|---:|---:|---:|---:|---:|
| $10^{19}$ | 0.162405 | 8.389 | 3.030398 | 87.33% |
| $10^{22}$ | 6.86104 | 226.492 | 2.145611 | 99.60% |
| $10^{24}$ | 11.965825 | 299.893 | 2.093379 | 2.30% |

最后一档选点达到当前 B1 支持网格的 $N,D$ 上界，且 1,176 个候选全部可行，因此 2.30% 的预算利用率反映有限候选域边界，不能外推为支持域外的最优配置。由题面代理成本 $C_{train}=6ND$、$C_{attn}=2\times10^{-4}NDL_{ctx}$，两项相等的临界上下文长度为 $L_{ctx}^{crit}=6/(2\times10^{-4})=30{,}000$ tokens；低于该长度时训练成本项较大，高于该长度时注意力成本项较大。C7 的 `max_position_embeddings` 是架构支持上限，不必然等于具体任务实际使用的窗口。

预算限制状态的有限网格判定为：五种上下文下，$10^{19}\to10^{22}$ 均未发生状态转换；$10^{22}\to10^{24}$ 均发生从预算受限到支持域边界受限的转换，配对参数 Bootstrap 为 1,000/1,000。该结论仅适用于固定 $Q=Q_0$、冻结 M0 与 B1 实测支持域。Q2 尚无可识别的独立质量响应与 B 侧配比效应，故完整经验 $Q/p$ 联合最优仍未识别；本报告给出的是范围受限的 $N/D$ 参考答案。

> **AI 辅助记录（竞赛定稿前处理）：** 本段文字由 OpenAI Codex（GPT-6-based；本地会话未暴露具体部署型号及模型发布日期）辅助整理，使用日期为 2026-09-26，数值引用本项目既有计算结果。参赛队须自行复核并以自身语言改写；用于答卷前按 2026 年竞赛规定核实并补全工具型号/版本与模型发布日期。
...
**来源：** [`Q3_fixedQ_results_integration_report.md`](Q3_fixedQ_results_integration_report.md)、[`q3_fixedQ_master_results.csv`](q3_fixedQ_master_results.csv)、[`q3_resource_transition.csv`](q3_resource_transition.csv)、[`M3_M4成本独立审计/m4_cost_audit_report.md`](M3_M4成本独立审计/m4_cost_audit_report.md) 及对应 reproduction manifests。
...
![图 1：固定 Q 主结果与相邻资源变化](figures/result_q3_fixedq_master_and_resource_transition.png)
...
![图 2：T2 成本—Loss Pareto、近优候选规模与边际收益](Q3_T2_近最优_Pareto_边际收益/figures/result_q3_t2_near_optimal_pareto_marginal_return.png)

**来源：** [`T2 报告`](Q3_T2_近最优_Pareto_边际收益/report.md)、[`近优表`](Q3_T2_近最优_Pareto_边际收益/q3_near_optimal.csv)、[`Pareto 表`](Q3_T2_近最优_Pareto_边际收益/q3_pareto.csv)、[`边际收益表`](Q3_T2_近最优_Pareto_边际收益/q3_marginal_return.csv)及其 reproduction manifest。15 个情景的 M0/M1 选点、M4 成本/预算标记与既有 Bootstrap 摘要均已复核。
...
**来源：** [`Q3_T3_稳健性与配置切换/report.md`](Q3_T3_稳健性与配置切换/report.md)、三张标准 CSV、[`reproduction_manifest.json`](Q3_T3_稳健性与配置切换/reproduction_manifest.json) 和 [`p1_smoke_receipt.json`](Q3_T3_稳健性与配置切换/p1_smoke_receipt.json)。标准包中的 P1 回执为 PASS；独立 T3 范围 P2 的核查范围与结论见 [T3 优化执行记录](../01_方案与设计/Q3-T3_Bootstrap稳定性与配置切换_优化执行记录.md)。该 P2 不覆盖 Q3 其他任务或整体冻结门禁。

![图 3：情景内 Bootstrap 选点稳定性](Q3_T3_稳健性与配置切换/figures/result_q3_t3_bootstrap_stability.png)

![图 4：相邻情景 N/D 配置切换](Q3_T3_稳健性与配置切换/figures/result_q3_t3_configuration_switch.png)

![图 5：相邻情景成本结构切换](Q3_T3_稳健性与配置切换/figures/result_q3_t3_cost_structure_switch.png)
...
本敏感性分析的 P1 与独立范围 P2 均通过；复核只覆盖本分析及报告引用的一致性，不等同于完整经验 Q/p 联合优化或竞赛提交级 QA。逐折参数、留出指标、3,528 条预测、360 个折内选点、复现命令和哈希见[专题报告](轨迹留一与函数形式敏感性/report.md)、[函数形式汇总表](轨迹留一与函数形式敏感性/q3_function_form_summary.csv)、[折内选点明细](轨迹留一与函数形式敏感性/q3_loto_scenario_selections.csv)及[复现清单](轨迹留一与函数形式敏感性/reproduction_manifest.json)。
...
**来源与复核：** [`Q3_Q0_baseline_sensitivity_report.md`](Q3_Q0_baseline_sensitivity_report.md)、[`q3_t4_conditional_beta_manifest.json`](q3_t4_conditional_beta_manifest.json)、`q3_betaQ_threshold.csv`、`q3_conditional_regime.csv`。T4 导出包的独立范围 P2 已通过；范围和证据见 [`T4_independent_P2_receipt.md`](T4_independent_P2_receipt.md)。

![图 6：条件 βQ 首次 N/D 重分配阈值及无有限阈值情景](figures/result_q3_t4_betaQ_scenario.png)
...
![图 7：五种上下文的候选可行占比与支持域饱和预算](预算饱和与有限网格切换/figures/result_q3_budget_saturation_finite_grid_switches.png)

**详细表格、事件定义、复现清单与适用范围：**[预算饱和与有限网格切换分析](预算饱和与有限网格切换/report.md)。

### G2：有限网格预算限制状态转换判定

题目要求判断预算跨量级时最优策略是否发生质变。仅观察到 N/D 网格点变化不足以作此判定，因此在固定 \(Q=Q_0\)、冻结 M0 与 B1 的 1,176 个实测候选点上，另定义有限支持域内预算限制状态。令 \(j_b^*=\arg\min_{j\in G}\widehat L_{j,b}\) 为第 \(b\) 个 Q2 参数 Bootstrap 复本在全支持域上的无预算最优点，并定义
\[
a_b(C,L)=\mathbf{1}\{C_{j_b^*}(L)>C\}.
\]
状态为 1 表示预算排除了无预算最优候选、限制了有限支持域最优解；状态为 0 表示该候选可行，当前网格的支持边界限制解。只有相邻预算之间的配对状态转换率达到预设的 95% 门槛，才判为有限网格预算限制状态转换。缩放律使用 B1 的十亿单位 \(n=N/10^9,d=D/10^9\)，成本则以实际参数数和 Token 数计算。1,000 个复本中 \(A,\alpha,B,\beta\) 均为正，且支持域有唯一联合最大 \((N,D)\) 点，故无预算最优点唯一；成本阈值 \(C_{\mathrm{sat}}(L)=\max_j C_j(L)\)。预算后命中联合最大点仅作一致性校验，不另算一份证据。

| 上下文 \(L\) | \(C_{\mathrm{sat}}\)（FLOPs） | \(10^{19}\to10^{22}\)：状态转换 / N/D 配置变化 | \(10^{22}\to10^{24}\)：状态转换 / N/D 配置变化 |
|---:|---:|---:|---:|
...
状态转换率表示冻结的 1,000 组参数 Bootstrap 复本中的稳定性，不是 1,000 个独立训练实验。由此可明确回答：在该固定-Q 有限支持域分支中，低→中预算只发生配置切换，预算限制状态不变；中→高预算在五种上下文下均发生 1→0 转换，达到 G2 的有限网格预算限制状态转移判据。十组预算相邻比较的成本份额均变化 0/1,000。完整结果、输入输出哈希、复核回执和复现脚本见 [G2 判定结果](G2_结构性转移判定_20260926/report.md)、[汇总表](G2_结构性转移判定_20260926/g2_transition_summary.csv)、[独立复核回执](G2_结构性转移判定_20260926/独立复核回执.md) 与 [G2 分析脚本](../03_代码/analyze_q3_g2_structural_transition.py)。

该判定只回答固定 \(Q=Q_0\)、B1 共同配比政策、冻结 M0 和已观测支持域内的预算限制状态。它不外推支持域外，也不识别完整 Q/p 联合最优；B6 与 B8 的 Q 响应方向相反且 Q 映射尚未验证，因此不据此声称统一的 Q/p 结构转移结论。

## 5. 结论、适用范围与未完成事项

当前证据支持的是：在固定 $Q=Q_0$、冻结 M0、B1 精确实测 N/D 支持网格上的经验参考优化、成本/预算覆盖审计与离散配置切换；进一步按有限支持域预算限制状态判据，识别出中→高预算的五个上下文均发生状态转换、低→中预算均未发生状态转换；在相同范围内描述参数 Bootstrap 稳定性、函数形式敏感性和整轨迹留一决策稳定性；并在未识别 $\beta_Q$ 的条件假设下给出 T4 阈值敏感性。留一预测支持当前 M0 继续作为本分支参考模型，但验证只覆盖 8 条 Pythia 轨迹和两种预先限定的形式约束。它们分别回答有限范围的问题，不能合并解释为完整 Q3 的联合优化结论。
...
为便于集中比较，本轮把 zwj 图集中的 P1–P4 和两张核心图补入 [Q3 追加候选图集](figures/候选_zwj追加_20260925/README.md)，包括与正文结果重叠的替代视图。它们暂存为候选图，不改变上文结论范围；各图适用范围和重复信息见图集索引。
