...
T2、预算饱和与有限网格切换，以及新增的 M0 函数形式/整轨迹留一敏感性，均已纳入本轮可识别结果报告。当前版本入口为 [rev2 独立复核结果包](冻结包_20260925_模型形式敏感性_独立复核/README.md)，其 [整包 P2 回执](冻结包_20260925_模型形式敏感性_独立复核/QA/P2_independent_release_receipt.md) 已通过。之前的 `冻结包_20260925/` 与预算分析包保留为历史快照，均未覆盖。冻结范围仍是固定 $Q=Q_0$、冻结 M0 与 B1 已观测 N/D 支持域上的可识别结果，以及 T4 条件 βQ 敏感性；完整经验 Q/p 联合优化未识别，不纳入正式结论。
