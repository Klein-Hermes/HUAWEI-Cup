...
def fig_c7_lengths():
    ...
    S.plot_heading(fig, "C7 模型最大上下文长度分布",
                   "柱上数字为模型数；虚线为由成本公式得到的临界长度 30,000 token。")
...
def fig_frontier():
    ...
    S.panel_label(ax2, "b")
    S.plot_heading(fig, "算力预算利用率与注意力成本份额",
                   "左图显示预算利用率，右图显示注意力开销份额；百分比标签便于比较不同上下文长度。")
...
def fig_cost_curve():
    ...
    S.plot_heading(fig, "不同质量成本函数下的增量算力曲线",
                   "纵轴为对数；水平虚线为算力预算，右侧标注预算等级，竖线标出质量基线 $Q_0$。")
...
def fig_conditional():
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 0.99),
               ncol=max(1, len(labels)), fontsize=8.2, title="算力预算",
               title_fontsize=8.2, frameon=False)
    # 图例置于整幅图上方，避免遮住指数型面板中的曲线。
    fig.subplots_adjust(left=0.15, right=0.98, top=0.78, bottom=0.20)
    # 三个面板里曲线占满全高，区内没有可靠的空位，故把注记放到坐标区外
    if not S.PAPER_MODE:
        fig.text(0.5, 0.035, f"$10^{{24}}$ 档的 {n_none} 个情景不存在 N/D 重分配，"
                             f"未在本图绘出", ha="center", va="bottom", fontsize=8,
                 color=S.C["ref"])
    S.plot_heading(fig, "质量成本形式对首次 N/D 重分配阈值的影响",
                   "每个面板对应一种成本函数，颜色区分算力预算；阈值越低表示越早触发资源重分配。",
                   axes_top=0.78, title_y=1.15, note_y=1.075)
