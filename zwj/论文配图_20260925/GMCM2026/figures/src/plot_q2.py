...
def fig_marginal(ck, sz):
    ...
    S.panel_label(ax2, "b")
    S.plot_heading(fig, "经典 M0 模型的参数量与数据量边际效应",
                   "左图随参数规模变化，右图随训练数据量变化；区间表示边际效应估计的不确定范围。")
...
def fig_elasticity(ck, sz):
    ...
    for a, (k, ylab) in zip((ax, ax2), spec):
        ...
        if not S.PAPER_MODE:
            a.text(0.5, 1.12,
                   f"95% 区间宽 ≤{wid/span*100:.2f}% 纵轴跨度，窄于线宽\n见下方放大图",
                   transform=a.transAxes, fontsize=7.6, color=S.C["ref"],
    ...
    fig.legend(handles=legend_handles, loc="lower center",
               bbox_to_anchor=(0.5, 0.015), ncol=3, fontsize=8.0,
               frameon=False, handlelength=1.5, columnspacing=1.0,
               handletextpad=0.5, labelspacing=0.35)
    fig.subplots_adjust(bottom=0.21)
    if S.PAPER_MODE:
        # 论文版沿用绘图版的图例排布，只去掉图题和说明，并用腾出的顶部空间放大图面。
        fig.subplots_adjust(top=0.97)
    else:
        S.plot_heading(fig, "M0 弹性估计与 95% 区间放大图",
                       "上排为完整弹性尺度，下排以自身中位数为零点放大 95% 区间；灰色带仅表示轨迹内 p10–p90 跨度。",
                       axes_top=0.82, title_y=0.985, note_y=0.92)
...
def fig_residual():
    ...
    S.plot_heading(fig, "B1 验证损失拟合的残差诊断",
                   "虚线为零残差基准；右上角给出残差偏倚与标准差。")
...
def fig_support():
    ...
    # 该信息改由 LaTeX 题注承担。
    S.plot_heading(fig, "B1 拟合所用的参数量与数据量支撑范围",
                   "每个方块对应一个训练检查点，颜色表示观测验证损失；8 个参数规模共享同一数据量网格。")
...
def fig_q_sign():
    ...
    S.plot_heading(fig, "固定 N–D 匹配单元中的 Loss–Q 斜率符号",
                   "条形表示负斜率与正斜率占比；条形两侧数字分别为对应的匹配单元数量。")
