...
def fig_model_gain():
    ...
    S.plot_heading(fig, "主模型相对训练均值基线的误差改善",
                   "左图比较嵌套交叉验证 RMSE，右图给出单纯形岭回归的相对改善幅度。")
...
def fig_conflict_effect():
    ...
    fig, (ax, ax2) = plt.subplots(
        1, 2, figsize=(7.4, 3.5), gridspec_kw=dict(width_ratios=[1.3, 1], wspace=0.48))
    ...
    for pos, arr, c in [(1, lo, S.C["ref"]), (2, hi, S.C["base"])]:
        ax2.scatter(np.median(arr), pos, marker="o", s=34, color="white",
                    edgecolors=c, lw=1.4, zorder=5)
    ax2.set_yticks([1, 2], [f"其余样本\n(中位 {np.median(lo):.4f})",
                            f"高冲突样本\n(中位 {np.median(hi):.4f})"], fontsize=8.4)
    ax2.set_xlabel("|$\\Delta Q$|")
    ax2.set_ylim(0.5, 2.6)
    ax2.set_xlim(0, max(np.quantile(hi, .99), np.quantile(lo, .99)) * 1.12)
    S.panel_label(ax2, "b")
    S.plot_heading(fig, "评分冲突度与两种评分结果分歧的关系",
                   "左图按样本密度着色并叠加分箱中位数；虚线为 q95 阈值，右图比较高冲突与其他样本。")
...
def fig_substitution_volcano():
    ...
    S.plot_heading(fig, "配比转移效应的方向确定性",
                   "灰点表示方向确定性不足 95%；蓝、橙点区分负向与正向效应，虚线标出零效应和方向无偏。")
