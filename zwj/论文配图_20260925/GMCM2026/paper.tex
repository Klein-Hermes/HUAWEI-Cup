...
% 【第一段：总体背景与研究思路。说明本文针对什么问题，用了哪几类方法，
%   整体是"物理机理 + 数据驱动"还是"优化求解"路线。】
...
针对问题四，
% ★ 已锁定 F 题：共 4 问，四段全保留（本题不需要删任何问题段）

% 【最后一段：模型的检验、灵敏度/稳健性分析结论，以及本文的创新点与适用边界】

\keywords{【模型名1】\quad 【模型名2】\quad 【模型名3】\quad 【模型名4】\quad 【方法名】}
...
挤占本可分配给参数与数据的算力预算。然而，规模扩张并不必然带来能力
提升。2026 年 2 月《Nature》刊载的一项研究提供了反例：参数量仅 80 亿的
OpenScholar-8B，在引文准确率与降低幻觉两项任务上超过了 GPT-4o。
...
算力在领域间的分配效果，进而影响最终的损失\cite{xia_regmix_2024}。

现有定量方法尚不足以回答“算力应投向何处”。广泛采用的标度律将交叉熵
...
数据量元数据，以及损失与测评得分之间的桥接数据。本文以【方法】将模型
能力增长分解为规模扩张与非规模技术进步两部分，分别量化二者的贡献占比，
再以【方法】预测未来 12 个月与 24 个月的开源模型能力前沿，并给出不确定性
区间。
...
\begin{tikzpicture}[
  ...
  \node[qbox, below=4mm of q3]   (q4) {问题四\\[1pt]\footnotesize【方法】};
...
% 【逐条按四项写：现实依据 → 简化对象 → 可能偏差 → 验证位置。
%   题目规定、变量定义、软件设置不算假设；
%   "数据准确""模型正确"不是可接受的无条件假设。
%   往届优秀论文普遍只写 3~4 条，但每条都有依据。】
\begin{enumerate}
\item 【假设一：依据……，将……近似为……；该处理可能在……处产生……偏差，
      后文通过……（图/表编号）检查。】
\item 【假设二】
\item 【假设三】
\end{enumerate}
...
% ---- 正文内容：下面每行替换为你的真实符号 ----
$i,\ j$      & 索引（如节点、架次、样本）            & --- \\
$N$          & 规模参数（写明具体取值，如“取 N=122”）  & --- \\
$x_{ij}$     & 决策变量：……                          & --- \\
$\theta$     & 模型参数：……                          & 【单位】 \\
$\mathcal{L}$ & 目标函数：……                         & --- \\
\multicolumn{3}{@{}l}{\footnotesize （提示：选题后删除本行，按需增删符号行）} \\
\end{longtable}
...
22 个指标经方向统一后的分布见图 \ref{fig:q1-inddist}。

\begin{figure}[htp!]
\centering
\includegraphics[width=.92\textwidth]{figures/q1-indicator-dist.pdf}
\caption{22 个质量指标经方向统一后的分布（红色标签为 7 个上游生成公式未追溯的字段）}
\label{fig:q1-inddist}
\end{figure}

\subsubsection{质量评分模型}
...
\texttt{rps\_lines\_numerical\_chars\_fraction} 的排序稳定性为 $0.9821$，
...
$95\%$ 效率对应的常数，其含义是残差在 $\delta$ 以内按平方损失处理，超出后转为
线性，从而把单个指标极端取值的影响限制在有界范围。样本分向域级聚合按域内 Huber
位置计算，并给出 $B_Q=2000$ 次记录级 bootstrap 的百分位 $95\%$ 区间。权重校准的
...
上述阈值属于描述性的上尾截断而非假设检验：以 $q_{95}$ 作阈值意味着高冲突样本恒占
$5\%$，这是定义使然，不能读作"$5\%$ 的样本确实存在冲突"。冲突度与阈值的联合作用
在于给出排序，而非作出绝对判定。冲突诊断与消解的完整流程见图
\ref{fig:q1-flowconf}。
...
在缺少外部真值的前提下强行加入只会使系数失去解释。因此本文改用配比单独建模，把
$Q$ 留给问题二的标度律处理。

除主模型外，另设四个对照与敏感性变体，即训练均值空模型（下界）、参考分量 OLS
（截距加 16 个分量、参考系数固定为 0）、Huber 岭回归（$\rho_{1.345}$，用于检验
异常损失的影响）与零友好的二次面（17 个一次项与 136 个两两乘积项，共 153 项）。
二次面仅作敏感性对照，153 个系数由 512 条配方估计，过拟合风险明显，故不参与模型族
选择。选模过程完全在 A4/A5 的分组交叉验证内完成，A6--A11 只用于评估冻结后的模型，
A12--A15 只用于与估计表比对一致性，不参与任何调参。配比与各损失目标之间的相关系数
见图 \ref{fig:q1-corr}。

\begin{figure}[htp!]
\centering
\includegraphics[width=.92\textwidth]{figures/q1-share-loss-corr.pdf}
\caption{配比与损失目标之间的相关系数结构（红色标签为 4 个无对应验证损失的配比域）}
\label{fig:q1-corr}
\end{figure}
...
区间另见图 \ref{fig:q1-transfer}。这一比较限定在标量层面，A2/A3 的特征文件不含原文
内容，原文层面的效度验证需借助 A18 的原始文本样例另行完成。
...
\subsubsection{冲突分析与消解结果}

各域的中位冲突度与 $q_{95}$ 阈值见表 \ref{tab:q1-conflict}。\texttt{arxiv} 的中位
冲突度最高（$0.4382$），\texttt{stackexchange} 最低（$0.2712$），两者相差 $0.167$；
扩展集的冲突度与同域 A1 高度接近，A2 为 $0.4393$、A3 为 $0.3236$。表中 A1 各域的
高冲突率恒为 $0.0500$，这是 $q_{95}$ 阈值的定义使然；A2 的 $0.0447$ 与 A3 的
$0.0514$ 则说明扩展集在套用 A1 阈值后，落入高冲突区的比例与原集基本一致。各域的
冲突度分布与阈值见图 \ref{fig:q1-confdist}。
...
\caption{各来源域的域内冲突度分布（虚线为该域的 $q_{95}$ 高冲突阈值，灰竖线为域内中位数）}
...
\caption{质量指标两两冲突强度的分布（琥珀色为 dsir 系列与一元熵的组合）}
...
\ref{tab:q1-conflict} 中 \texttt{book} 唯一非 $0.05$ 的高冲突率互为印证。扩展集上
的外部复核结果见图 \ref{fig:q1-confext}。

\begin{figure}[htp!]
\centering
\includegraphics[width=.92\textwidth]{figures/q1-conflict-external.pdf}
\caption{冲突指标在扩展集上的复核结果。两者是量纲不同的量，故分列两栏：
(a) 域内中位冲突度之差；(b) 高冲突率之差。区间跨零即无可检出差异}
...
\caption{检验尺度对预测误差与秩相关的影响：误差随尺度恶化而秩相关保持}
...
\caption{配比替代效应的关联系数结构（系数满足 $\sum_j\beta_{ij}=0$，参考域固定为 0）}
\label{fig:q1-subst}
\end{figure}
各组合的方向确定性见图 \ref{fig:q1-substitution-volcano}。

\begin{figure}[htp!]
\centering
\includegraphics[width=.92\textwidth]{figures/q1-substitution-volcano.pdf}
\caption{配比替代效应的方向确定性（1768 个目标与域对组合中 895 个方向稳定）}
...
其中 24 条代表性样本用于候选比较，6 条诊断性样本用于案例检查。两位评审的总体质量
...
也不构成对英文原文的直接验证。A2/A3 的原文链接仅覆盖部分记录，未链接记录不纳入
...
二次面的表现亦予报告，但不作事后改选。其嵌套 CV 宏平均标准化 RMSE 为 $0.58901$，
优于主模型的 $0.63305$，检验集 $R^2$ 也更高（$0.6817$ 对 $0.6189$）。但 153 个
系数由 512 条配方估计，且不区分结构零与舍入零（511/512 行含零），过拟合风险明确。
按选模纪律，主模型在选模阶段即已冻结，不允许在看到检验集结果后更改，故二次面仅作
敏感性对照保留。域序对预处理场景的稳健性见图 \ref{fig:q1-rank}。

\begin{figure}[htp!]
\centering
\includegraphics[width=.92\textwidth]{figures/q1-rank-robustness.pdf}
\caption{A1 七域排序在预设场景下的稳健性}
\label{fig:q1-rank}
\end{figure}
...
\caption{B1 的设计支撑：8 个参数量取值与 147 个训练检查点构成的交叉设计，颜色为
观测验证损失。同一数据量在各规模列之间水平对齐，是 $\alpha$ 与 $\beta$ 可分别
识别的依据；点云的外沿即该拟合的支持边界}
...
\centering
\renewcommand\arraystretch{1.2}
\caption{经典标度律 $L=E+AN^{-\alpha}+BD^{-\beta}$ 在 B1 上的参数估计}\label{tab:q2-params}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lccc@{}}
\toprule
...
\caption{B1 的残差结构：残差均值约为零（无系统偏倚），但离散度随拟合损失上升而
扩大（异方差）}
...
\centering
\renewcommand\arraystretch{1.2}
\caption{B1 中位点处的边际效应与弹性（括号内为 $95\%$ 整轨迹 bootstrap 区间）}\label{tab:q2-elas}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llcc@{}}
\toprule
...
而非最优配比本身。边际效应 $\partial L/\partial D$ 在八条轨迹上完全相同（均为
$-5.8526\times10^{-4}$），因为 M0 形式中该项只依赖 $D$ 而不依赖 $N$，这是该函数
形式的结构性含义而非拟合巧合。
...
改以文字标注，以免留下一条读者看不见的置信带。

\begin{figure}[htp!]
\centering
\includegraphics[width=.94\textwidth]{figures/q2-m0-marginal.pdf}
\caption{两个规模方向的边际效应：(a) $|\partial L/\partial N|$ 随模型规模的变化；
(b) $|\partial L/\partial D|$ 随训练数据量的变化}
...
\caption{两个无量纲弹性随模型规模的变化。上排为全尺度：(a) $\varepsilon_N$、(b)
$\varepsilon_D$，圆点为轨迹中位数，灰棒为同一轨迹内检查点的 p10--p90 跨度；下排
(c)(d) 把 $95\%$ 整轨迹 bootstrap 区间以各点自身的中位数为零点放大显示——该区间
仅占纵轴跨度的 $0.07\%$ 与 $0.14\%$，在全尺度图中窄于线宽而不可见}
...
\caption{冻结 M0 下两个弹性沿训练数据量的剖面（按参数量分面）。主图为真实弹性尺度；
每个主图下方的窄条把 $95\%$ 整轨迹 bootstrap 区间相对估计值的偏差放大显示，单位为
$10^{-5}$，未改变原始区间。区间基于 8 条完整 Pythia 轨迹，独立轨迹数偏少，仅作
稳定性提示；弹性是冻结 M0 的模型蕴含量，不代表配比或质量的效应}
...
\centering
\renewcommand\arraystretch{1.2}
\caption{B6/B7 半合成数据上的质量指数估计}\label{tab:q2-qfit}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lcccccc@{}}
\toprule
数据集 & 行数 & $\kappa$ & $\gamma=\kappa/\beta$ & $\varepsilon_Q$ 中位数 & 留一 Q 水平 RMSE & 相对 M0 改善 \\
\midrule
B6 & 360 & 0.085033 & 1.216453 & $-0.054048$ & 0.089840 & 0.145135 \\
B7 & 450 & 0.098000 & 1.179775 & $-0.052055$ & 0.084316 & 0.129540 \\
\bottomrule
\end{tabular*}
\end{table}

式中，"相对 M0 改善"一列为同一半合成网格上含质量项与不含质量项两个模型的留一
Q 水平 RMSE 对照，含质量项的模型误差更低，说明半合成网格确实携带了 $Q$ 方向的
信号。取共同网格中心（$N=1$B、$D=150$B、$Q=0.6$）为工作点，把 $Q$ 提高 $0.1$
后维持损失不变所需的参数量为

\begin{table}[htp!]
\centering
\renewcommand\arraystretch{1.2}
\caption{$Q$ 提高 $0.1$ 的等损失参数量兑换（工作点 $N=1$B、$D=150$B、$Q=0.6$）}\label{tab:q2-trade}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lccc@{}}
\toprule
...
\caption{匹配 $N$–$D$ 单元内 Loss 对 $Q$ 的斜率符号分布：B6/B7 的 90 个单元全为负，
B8 的校准与外推共 150 个单元全为正，两组方向完全分离}
\label{fig:q2-qsign}
\end{figure}
...
\caption{A 侧配比模型在同尺度与跨尺度切分上的逐目标表现：(a) 逐目标 RMSE；
(b) 逐目标 $R^2$（symlog 刻度，虚线为零参考）。点代表 13 个领域目标，菱形为其
算术平均。点不是独立训练重复，故不配置区间；该切分不构成新增盲测，跨尺度表现只
说明迁移限制，不证明 B 侧的配比效应}
\label{fig:q2-a-side}
\end{figure}
...
\caption{C7 上下文长度的分布与临界长度 30000 的位置（横轴为对数刻度）}
\label{fig:q3-c7}
\end{figure}
...
\caption{15 个情景的预算利用率与成本份额（(a) 虚线为预算耗尽线；(b) 虚线为份额过半线）}
...
\caption{离散预算与上下文情景下的资源配置、成本构成与 M0 预测损失。(a) 选中的
参数量；(b) 选中的数据量；(c) 训练／注意力／质量三类成本的百分百堆叠——固定
$Q=Q_0$ 时质量成本恒为零；(d) M0 预测验证损失。空心圈表示该点触及 B1 的 $N$、$D$
支持上界，线段仅连接三个给定预算档位。22 对相邻变化仅作描述性比较，不使用
bootstrap 或 KKT 判据，也不构成结构转移结论}
...
\centering
\renewcommand\arraystretch{1.2}
\caption{质量由 $Q_0$ 提升至 $Q=1$ 的增量算力（$D=146.801\times10^{9}$ token）}\label{tab:q3-cq}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}lccc@{}}
\toprule
...
\caption{三种质量成本函数的增量算力曲线（$D$ 取 B1 数据量的最小值、中位值与最大值）}
\label{fig:q3-cost}
\end{figure}
...
\caption{条件性质量响应模型下首次触发 $N$、$D$ 重分配的质量响应阈值 $\beta_Q$
（三个面板分别为三种成本函数，横轴为上下文长度，纵轴为对数刻度）。
$10^{24}$ 档的 15 个情景不存在重分配，未绘出}
\label{fig:q3-cond}
\end{figure}
...
\caption{轨迹级 cluster bootstrap 下的配置选择稳定性。把问题二已有的 1000 次轨迹
级 bootstrap 参数传播到 15 个固定 $Q$ 情景：(a) 参数量、(b) 数据量的 $95\%$ 区间
在全部情景中退化为单点，未作视觉放大；(c) M0 预测损失的 $2.5\%$／中位数／$97.5\%$
分位；(d) 配置切换概率，恒为 $0\%$。该 bootstrap 仅基于 8 条 B1 轨迹，只反映既定
M0 形式与支持网格内的参数重抽样稳定性，不覆盖模型形式误差、外部迁移或配比／质量
响应}
...
\caption{$q_{\mathrm{huber}}$ 与 $q_{\mathrm{eq}}$ 两个质量基线的固定-$Q$ 敏感性
对照。(a) 两个基线数值分别为 $0.4984781048$ 与 $0.4962230100$；(b)(c)(d) 分别为
两个分支下的参数量、数据量与 M0 预测损失，两组曲线在每个情景上完全重合（最大绝对
差为 $0$），故部分序列被另一组遮住。该对照不表示质量对损失的实证收益，也不构成
配比与质量的联合优化}
\label{fig:q3-qbase}
\end{figure}
...
% 【· 关键参数 ±20% 扰动，画曲线/九联图
%   · 随机算法多种子重复实验（给出均值±标准差，配箱线图）
%   · 看结论在扰动范围内是否保持稳定，给出适用边界】

\subsection{消融实验}

% 【★ 本模板最推荐的加分项。逐项去掉再跑一遍：
%   · 去掉某个模块 / 某个损失项 / 某个预处理步骤 / 某类特征
%   · 换成更朴素的替代方案
%   用一张表给出：完整模型 vs 各消融变体的主指标 + 代价
%   结论要能回答"我加的这一层到底贡献了多少"】

\subsection{误差分解与失效案例}

% 【· 把总误差拆成来源（口径差异 / 参数选择 / 数据划分 / 随机种子）
%   · 最差工况单独点名并归因
%   · 诚实写出失效条件：在什么范围内结论成立、超出后失效】
...
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llll@{}}
  ...
  工具名称 & 版本 & 提供方 & 使用时段 \\
  \midrule
  【例：Claude Code】 & 【例：Fable 5】 & 【例：Anthropic】 & 【例：09-23 至 09-26】 \\
  【工具二】 & 【版本号】 & 【提供方】 & 【时段】 \\
  \bottomrule
\end{tabular*}
...
\begin{tabularx}{\textwidth}{lXX}
  ...
  \midrule
  概念理解 &
  【例：协助梳理【某方法】的基本原理与适用条件】 &
  【例：查阅【文献】核实其推导；确认适用条件在本题数据上成立】 \\
  资料整理 &
  【例：协助检索【某领域】的公开文献线索】 &
  【例：逐篇核对原文与出处，正文引用均以正式发表文献为准】 \\
  代码调试 &
  【例：协助排查【某函数】的报错与数值稳定性问题】 &
  【例：自行完成算法设计与实现；对 AI 建议逐条测试，用【某验证】确认结果正确】 \\
  【其他环节】 & 【具体贡献】 & 【独立工作】 \\
  \bottomrule
\end{tabularx}
...
\begin{enumerate}
  ...
  \item \textbf{独立复算}：凡涉及数值结果的内容，
        ...
        论文报告的每个数值均可追溯至 \texttt{state/数字登记表.csv} 登记的结果。
  \item \textbf{处理说明}：【填写具体例子——例如“AI 曾建议采用某方法，
        经验证在本数据上不满足某前提，故改用另一方法”，
        或“AI 给出的某公式存在某错误，经推导修正为……”。
        真实的修正例子比空泛声明更有说服力。】
\end{enumerate}
